...
class UserDAO:
    ...
    def save_assessment(self, username: str, response: AssessmentResponse):
        ...
        for question in response.questions:
            if not question.answer:
                logger.warning(
                    f"问题 {username} {response.assessment_id} {question.question_id} 无答案，跳过"
                )
                continue
            answer_type = "choice" if type(question.answer).__name__ == "ChoiceAnswer" else "text"
            try:
                answer_value = json.dumps(question.answer.dict(exclude_none=True))
                logger.debug(
                    f"保存问题 {username} {response.assessment_id} {question.question_id}: type={answer_type}, value={answer_value}"
                )
            except Exception as e:
                logger.error(
                    f"序列化答案失败 {username} {response.assessment_id} {question.question_id}: {e}"
                )
                continue
            ...
            logger.debug(
                f"保存问题 {question.question_id}, 结果: {result.upserted_id or result.modified_count}"
            )
    ...
    def get_assessments(self, username: str) -> list[AssessmentResponse]:
        ...
        for assessment_id in assessment_ids:
            ...
            logger.debug(f"获取 {username} 的测评 {assessment_id}，问题数量: {len(questions_data)}")
            questions = []
            for q in questions_data:
                answer_value = q.get("answer_value")
                answer_type = q.get("answer_type")
                logger.debug(
                    f"解析问题 {q['question_id']}，answer_type={answer_type}, answer_value={answer_value}"
                )
                answer = None
                if answer_value and answer_type:
                    try:
                        answer_data = json.loads(answer_value)
                        if answer_type == "text":
                            answer = TextAnswer(**answer_data)
                        elif answer_type == "choice":
                            answer = ChoiceAnswer(**answer_data)
                        else:
                            logger.warning(
                                f"未知 answer_type: {answer_type} for {q['question_id']}"
                            )
                    except (json.JSONDecodeError, ValueError) as e:
                        logger.error(
                            f"解析答案失败 {username} {assessment_id} {q['question_id']}: {e}, answer_value={answer_value}"
                        )
                questions.append(QuestionResponse(question_id=q["question_id"], answer=answer))
            assessments.append(AssessmentResponse(assessment_id=assessment_id, questions=questions))
        logger.info(f"用户 {username} 测评数量: {len(assessments)}")
        return assessments
